...
public class Main4Activity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final int mImageThumbSize = 200;
        final int mImageThumbSpacing = 1;

        final GridView photoWall = findViewById(R.id.photo_wall);
        adapter = new PhotoWallAdapter(this, 0, Images.imageThumbUrls, photoWall);
        photoWall.setAdapter(adapter);
        //
        photoWall.getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
            @Override
            public void onGlobalLayout() {
                Log.e(TAG, "onGlobalLayout");
                final int numColumns = (int) Math.floor(photoWall.getWidth() / (mImageThumbSize + mImageThumbSpacing));
                if (numColumns > 0) {
                    int columnWidth = (photoWall.getWidth() / numColumns) - mImageThumbSpacing;
                    adapter.setItemHeight(columnWidth);
                    photoWall.getViewTreeObserver().removeGlobalOnLayoutListener(this);
                }
            }
        });
